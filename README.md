# Взаимодействие сервисов при создании заказа

Репозиторий с описанием взаимодействия микросервисов (схемы и Helm-чарт).

- **GitHub:** https://github.com/n-mark/interaction

> Примечание: этот репозиторий — документация и Helm-чарт, отдельного Docker-образа не имеет.
...
- **Идемпотентность** потребителей (проверка `eventid`) — брокер может доставлять сообщения повторно (at-least-once). (TODO)
- **Transactional outbox** в продюсерах чтобы гарантировать: запись в БД и отправка в брокер — атомарны. (TODO)
